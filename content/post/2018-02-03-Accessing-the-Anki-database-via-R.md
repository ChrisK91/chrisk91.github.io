...
aliases:
  - /2018/02/03/Accessing-the-Anki-database-via-R.html
outline: true
---
...
In order to have a save working environment, I *strongly recommend* working on a copy of your database. You can find it (on Windows) in ```%appdata%/anki2/YOUR_PROFILE```. Copy the file ```collection.anki2``` to a new folder, and create a ```.R``` script in the same location. I tend to use [RStudio](https://www.rstudio.com/) as my environment when coding, since it has some nifty features (e.g. run the selected line with Ctrl + R and inline documentation view). In RStudio, you can also set the working directory to the location of your code file via *Session &rarr; Set Working Directory &rarr; To Source File Location*

I will use the following packages in sections below: RSQLite, DBI (both to access the database), jsonlite (to parse some data) and ggplot2 (to create some graphs). If you don't have experience with ggplot2, you might want check out [this short introduction](http://r-statistics.co/ggplot2-Tutorial-With-R.html).

You can install the required packages and dependencies by running the following command:

{{< code lang="R" >}}install.packages(c("RSQLite", "DBI", "jsonlite", "ggplot2")){{< /code >}}

Now obviously, we want to load these packages:

{{< code lang="R" >}}require(RSQLite)
require(DBI)
require(ggplot2)
...
{{< code lang="R">}}connection <- dbConnect(SQLite(), dbname="collection.anki2")
...
{{< code lang="R" >}}'data.frame':	1 obs. of  13 variables:
 $ id    : int 1
 $ crt   : int 1477461600
 ...
 $ tags  : chr "{\"Gyn_Gutartige_EndometriumCA\": 0, \"Derma_Allgemein\": 0, \"Sono\": 0, \"Uro\": 0, \"Innere_EKG\": 0, \"Uro_"| __truncated__{{< /code >}}
...
{{< code lang="R" >}}collectionNames <- fromJSON(txt=collectionData$decks[1]) # use the first row in the 'decks' column{{< /code >}}
...
{{< code lang="R" >}}getDeckName <- function(x) collectionNames[[x]]$name{{< /code >}}
...
{{< code lang="R" >}}cardData$deck <- factor(mapply(getDeckName, as.character(cardData$did))) #perform lookup of deck names{{< /code >}}
...
{{< code lang="R" >}}getDeckNameFromCid <- function(x)
  {
...
revisionData$deck <- factor(mapply(getDeckNameFromCid, revisionData$cid)){{< /code >}}

Depending on the amount of reviews, the lookup might take a while.
...
{{< code lang="R" >}}ggplot(revisionData, aes(time / 1000)) + geom_histogram(binwidth = 10) + xlab("Time spent reviewing a single card\n(seconds)") + scale_y_log10(){{< /code >}}

{{< fig src="/images/anki_stats/histogram.png" >}}{{< /fig >}}
...
{{< code lang="R" >}}ggplot(revisionData, aes(time / 1000)) + geom_histogram(binwidth = 10) + xlim(0, 400) + xlab("time spent reviewing a single card\n(seconds)") + scale_y_log10() + ggtitle("Review times") + facet_wrap(~ deck){{< /code >}}

{{< fig src="/images/anki_stats/histogram_decks.png" >}}{{< /fig >}}
...
Finally, let's see how our decks are composed. I want to check out, how my intervals are distributed across my different decks:

{{< code lang="R" >}}ggplot(cardData, aes(ivl)) + geom_histogram(binwidth = 10) + facet_wrap(~ deck, scales = "free_y"){{< /code >}}

{{< fig src="/images/anki_stats/histogram_review_times.png" >}}{{< /fig >}}
...
{{< code lang="R" >}}plotWidth <- floor(sqrt(nrow(cardData))) # our final dimension should be a square
...
ggplot(cardData, aes(X, Y)) + geom_tile(aes(fill = ivl)) + scale_fill_gradient2(mid = "#dd6a27", high = "#12e824"){{< /code >}}

{{< fig src="/images/anki_stats/review_heatmap.png" >}}{{< /fig >}}
...
{{< fig src="/images/anki_stats/review_heatmap_filtered.png" >}}{{< /fig >}}


## Summary

We have imported the Anki data into R. By using only a fraction of the available data, we were able to get some insights into the data. The full code to create these graphics is available on GitHub [here](https://gist.github.com/ChrisK91/c29177116b23660111a28cba1a4d186a).

If you create a cool graphic, let me know via [Twitter](https://www.twitter.com/ChrisK91).

[Here is another thing I've come up with.](https://streamable.com/s/7b5rl/qfyltr) I've animated how the intervals of my cards have changed over the time. In the beginning, the maximum interval was quite limited. Once I've lifted that, you can see the intervals exploding. This was also done purely in R (and ffmpeg to get the video).
## Further reading

- [A short introduction into the powerfull ggplot2 package](http://r-statistics.co/ggplot2-Tutorial-With-R.html)
- [ggthemes, a package with some built in themes to make your plots look nicer](https://github.com/jrnold/ggthemes)
- [A overview of the styles that come with ggthemes](https://cran.r-project.org/web/packages/ggthemes/vignettes/ggthemes.html)
- [A discussion of the Anki database schema, from the Anki Android project](https://github.com/ankidroid/Anki-Android/wiki/Database-Structure)
- [SQLite Browser, allows you to browse the database directly](http://sqlitebrowser.org/)
